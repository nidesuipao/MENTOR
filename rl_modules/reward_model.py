...
import os
import time
from human_label import Human_Label_Manager
import random
...
class RewardModel:
    def __init__(self, ds, da,
                 ...
                 env_maker=None, max_size=100, activation='tanh', capacity=600,
        ...
        self.HL_Manager = Human_Label_Manager("four_room", "123")
        self.idx = 0
    ...
    def construct_ensemble(self):
        for i in range(self.de):
            model = nn.Sequential(*gen_net(in_size=self.ds + self.da,
                                           out_size=1, H=512, n_layers=5,
                                           activation=self.activation)).float().to(device)
            self.ensemble.append(model)
            self.paramlst.extend(model.parameters())

        self.opt = torch.optim.Adam(self.paramlst, lr=self.lr, weight_decay=0.001)
    ...
    def add_data(self, obs, act, rew):
        ...
        if len(self.inputs) < 200:
            self.inputs.append(flat_input)
            self.targets.append(flat_target)
        else:
            self.inputs[self.idx] = flat_input
            self.targets[self.idx] = flat_target
    ...
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2):
        ...
        # reward_diff = sum_r_t_1 - sum_r_t_2
        # print(reward_diff)
        # for i in range(len(reward_diff)):
        #     if abs(reward_diff[i]) < 0.03:
        #         rational_labels[i] = random.randint(0, 1)
    ...
    def kcenter_entropy_sampling(self):
        ...
        # get final queries based on uncertainty
        entropy, _ = self.get_entropy(sa_t_1.reshape(num_init, 1, -1), sa_t_2.reshape(num_init, 1, -1))
        ...
        size_0 = sa_t_2.shape[0]
        if len(labels) > 0:
            self.put_queries(sa_t_1.reshape(size_0, 1, -1), sa_t_2.reshape(size_0, 1, -1),
                             labels.reshape(size_0, -1))

        return len(labels)

    def uniform_sampling(self, step):
        # get queries
        sa_t_1, sa_t_2, r_t_1, r_t_2 = self.get_queries(
        ...
        if self.hl:
            labels = self.HL_Manager.data_store_and_pic_gen(sa_t_1, sa_t_2)

        if len(labels) > 0:
            self.put_queries(sa_t_1.reshape(self.mb_size, 1, -1), sa_t_2.reshape(self.mb_size, 1, -1),
                             labels.reshape(self.mb_size, -1))
    ...
    def entropy_sampling(self):
        ...
        entropy, _ = self.get_entropy(sa_t_1.reshape(self.mb_size * self.large_batch, 1, -1), sa_t_2.reshape(self.mb_size * self.large_batch, 1, -1))
        # print(sa_t_1.shape)
        # print(entropy.shape)
