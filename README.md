...
```bash
mpirun -np 1 python -u train_sac_multigoal.py --env-name='FetchReach-v1' --n-cycles=10 2>&1 | tee reach.log
```
2. train the **FetchPush-v1**:
```bash
mpirun -np 8 python -u train_sac_multigoal.py --env-name='FetchPush-v1' 2>&1 | tee push.log
```
3. train the **FetchPickAndPlace-v1**:
```bash
mpirun -np 16 python -u train_sac_multigoal.py --env-name='FetchPickAndPlace-v1' 2>&1 | tee pick.log
```
4. train the **FetchSlide-v1**:
```bash
mpirun -np 8 python -u train_sac_multigoal.py --env-name='FetchSlide-v1' --n-epochs=200 2>&1 | tee slide.log
```
